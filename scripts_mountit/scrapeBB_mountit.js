    ...
    while (attempt < retries) {
        try {
            ...
            const data = await page.evaluate(() => {
                const outOfStockMessage = document.querySelector('.inactive-product-message.text-danger')?.innerText.trim();
                const soldOut = [...document.querySelectorAll('div strong')].some(el => el.innerText.trim() === 'Sold Out');
            
                return {
                    title: document.querySelector('div.sku-title h1')?.innerText.trim() || 'Not Found',
                    price: document.querySelector('div.priceView-hero-price span[aria-hidden="true"]')?.innerText.trim() || 'Not Found',
                    stock: soldOut || (outOfStockMessage === "This item is no longer available in new condition.") ? "False" : "True"
                };
            });
...
async function fetchAllProductsData(productList) {
    ...
    for (let i = 0; i < productList.length; i++) {
        ...
        const url = `https://www.bestbuy.com/site/${product.itemId}.p?intl=nosplash&skuId=${product.itemId}`;
